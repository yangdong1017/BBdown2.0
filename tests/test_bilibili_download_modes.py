...
class BilibiliDownloadPageTests(unittest.TestCase):
    ...
    def test_standard_link_is_displayed_and_opened_in_browser(self) -> None:
        ...
        with (
            ...
            self.assertTrue(page.right_panel.isAncestorOf(page.login_state_label))
    ...
    def test_right_settings_panel_can_collapse_and_expand(self) -> None:
        ...
        with (
            ...
            self.assertTrue(page.right_panel_expanded)
            self.assertFalse(page.right_panel.isHidden())

            page.right_panel_toggle.click()
            QTest.qWait(260)
            self.assertFalse(page.right_panel_expanded)
            self.assertTrue(page.right_panel.isHidden())

            page.right_panel_toggle.click()
            QTest.qWait(260)
            self.assertTrue(page.right_panel_expanded)
            self.assertFalse(page.right_panel.isHidden())
            self.assertGreater(page.right_panel.width(), 0)
            page.close()
            page.deleteLater()
